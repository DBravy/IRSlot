...
class DataProcessConfig(BaseModel):
    ...
    output_dir: str
    arc_version: str = "agi1"  # "agi1" or "agi2"
    subsets: List[str] = None  # Auto-set based on arc_version if None
    test_set_name: str = None  # Auto-set based on arc_version if None
...
def convert_dataset(config: DataProcessConfig):
    ...
    # Auto-configure subsets based on ARC version
    if config.arc_version == "agi1":
        # ARC-AGI-1: Only use training and evaluation (no "2" suffix)
        if config.subsets is None:
            config.subsets = ["training", "evaluation"]
        if config.test_set_name is None:
            config.test_set_name = "evaluation"
        print(f"=== ARC-AGI-1 Configuration ===")
        print(f"Training/Eval subsets: {config.subsets}")
    elif config.arc_version == "agi2":
        # ARC-AGI-2: Use ALL training data (including ARC-AGI-1 training + eval)
        # Only evaluate on evaluation2, so we can safely use all other data for training
        if config.subsets is None:
            config.subsets = ["training", "training2", "concept", "evaluation", "evaluation2"]
        if config.test_set_name is None:
            config.test_set_name = "evaluation2"
        print(f"=== ARC-AGI-2 Configuration ===")
        print(f"Using ALL available data (including ARC-AGI-1 training + evaluation)")
        print(f"Training/Eval subsets: {config.subsets}")
    elif config.arc_version == "both":
        # Use all datasets (for testing/experimentation only - not recommended for evaluation)
        if config.subsets is None:
            config.subsets = ["training", "training2", "evaluation", "evaluation2", "concept"]
        if config.test_set_name is None:
            config.test_set_name = "evaluation"
        print(f"=== WARNING: Using BOTH ARC-AGI-1 and ARC-AGI-2 ===")
        print(f"Training/Eval subsets: {config.subsets}")
        print("⚠️  WARNING: This is NOT recommended for proper evaluation due to data leakage!")
        print("⚠️  ARC-AGI-2 training contains some ARC-AGI-1 evaluation puzzles!")
    else:
        raise ValueError(f"Invalid arc_version: {config.arc_version}. Must be 'agi1', 'agi2', or 'both'")

    print(f"Test set: {config.test_set_name}")
    print()
